...
## 3. Extra Balance Features
   - **Swap Uneven Teams** - Swaps players between the two teams to even out the player count.
      ...
      - Players swapped indicated by "MOVED".       
   ...
   - **Compensate** - Assuming spec types are as already even as they can be.
      - Only works if weight diff is greater than 1.
      - First takes the highest spec type weight difference = x.
      - Then looks at different spec types and finds a player on higher weighted team to swap with another player on the lower weighted team who is lower weight and chooses the highest difference closest to (x/2 + Bonus).
